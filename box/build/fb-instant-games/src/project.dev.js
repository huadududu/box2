      ...
      efficiencyAdd: function efficiencyAdd(id) {
        ...
        if (1002 == conf.costtype && Global.gem >= conf.cost) {
          ...
          if (conf.coin > 0) {
            ...
            var timestamp1 = Date.parse(new Date()) / 1e3;
          ...
          if (conf.jumptime > 0) {
            for (var i = this.hammerStart; i <= this.hammerEnd; i++) if (void 0 != Global.hammer[i]) {
              var attr = AttributeConfig[Global.hammer[i].attribute];
              addgold += 60 * conf.jumptime * 50 / attr.time * attr.att;
            }
            var myinfo = {};
            if (gem >= 0) {
              myinfo.gem = gem;
  ...
  GameMenuController: [ function(require, module, exports) {
    ...
    cc.Class({
      ...
      addUIBottom: function addUIBottom() {
        ...
        single < 7 && (startpos += 156 * (7 - single));
      ...
      updateDate: function updateDate(data) {
        ...
        "level" != name && "gold" != name && "gem" != name || this.updateButtom();
      },
      scrollEvent: function scrollEvent(sender, event) {
        var thispos = sender.getScrollOffset();
        var movex = -thispos.x;
        var num1 = 156 * this.itemlistNum[0] - 6;
        var num2 = num1 + 156 * this.itemlistNum[1] - 6;
      ...
      radioButtonClicked: function radioButtonClicked(toggle) {
        ...
        switch (index) {
         ...
         case 1:
          var num0 = 156 * this.itemlistNum[0];
          this.scrollView.scrollToOffset(cc.p(num0, 0), .2);
          break;

         case 2:
          var num1 = 156 * (this.itemlistNum[0] + this.itemlistNum[1]);
      ...
      var cls = cc.Class({
        ...
        initInfo: function initInfo() {
          ...
          lc.get("freindsInfo", [], function(v) {
            this.freindsInfo = "array" == typeof v ? v : [];
    ...
    cc.Class({
      ...
      setIcon: function setIcon() {
        ...
        if (bool) {
          ...
          pngname = null != pngicon ? pngicon : config[this.type][this.ID].icon;
      ...
      setDesc_1: function setDesc_1() {
        var desc = void 0;
        var attnum = void 0;
        if (1 != this.type) desc = config[this.type][this.ID].desc; else {
          attnum = null != Global.hammer[this.ID] ? Global.hammer[this.ID].attribute : config[this.type][this.ID].attribute;
        ...
        LanguageConfig[desc] && LanguageConfig[desc][this.language] && (desc = LanguageConfig[desc][this.language]);
        desc = 0 == this.type ? config[this.type][this.ID].speed > 0 ? this.formatPrint(desc, config[this.type][this.ID].speed, config[this.type][this.ID].coin) : this.formatPrint(desc, config[this.type][this.ID].coin) : 1 == this.type ? this.formatPrint(desc, AttributeConfig[attnum].att, AttributeConfig[attnum].time) : config[this.type][this.ID].time > 0 ? this.formatPrint(desc, config[this.type][this.ID].time) : this.formatPrint(desc, config[this.type][this.ID].jumptime);
      ...
      setTitle: function setTitle() {
        ...
        2 == this.type && config[this.type][this.ID].coin > 0 && (title = this.formatPrint(title, config[this.type][this.ID].coin));
      ...
      setType1BtnState: function setType1BtnState() {
        ...
        if ("unlock" in conf) {
          ...
          if (-1 == thisID) return;
          var active = false;
          if (null == Global.hammer[this.ID] || void 0 == Global.hammer[this.ID]) {
            ...
            if (0 == thisID) {
              ...
              0 == Global.inviteFriends ? this.btntext.string = LanguageConfig["10023"][this.language] : confArry[1] > Global.inviteFriends ? this.btntext.string = Global.inviteFriends + "／" + confArry[1] : this.btntext.string = LanguageConfig["10020"][this.language];
              this.title.node.active = false;
            }
          } else if (-1 == AttributeConfig[Global.hammer[this.ID].attribute].next) {
            this.ButtonState(0);
      ...
      formatPrint: function formatPrint() {
        var num = arguments.length;
        var oStr = arguments[0];
        for (var i = 1; i < num; i++) oStr = oStr.replace(/s%/, arguments[i]);
        return oStr;
      },
  ...
  UpgradView: [ function(require, module, exports) {
    ...
    cc.Class({
      ...
      onEnable: function onEnable() {
        ...
        "uplevel" == Global.typebtn ? this.addlevel() : "skip" == Global.typebtn ? this.skipReward() : this.outlineReward();
      },
...
}, {}, [ "BoxController", "GameMenuController", "ParticleSystemCenter", "SkeletonDataCenter", "SpriteFrameCenter", "smController", "FBPlugin", "AcceleratorConfig", "AttributeConfig", "BlockBig", "BlockBigFactory", "BlockConfig", "BlockSmall", "BlockSmallFactory", "BoxConfig", "BoxConfig1", "CycleConfig", "EfficiencyConfig", "GameConfig", "GameState", "GameType", "Global", "ItemConfig", "LanguageConfig", "LevelConfig", "MarginsBig", "MarginsSmall", "RewardConfig", "RewardItem", "RewardItemFactory", "StageConfig", "ToolConfig", "UIBottom", "UIBottomFactory", "BingLog", "GameUtils", "JSExtends", "LocalStorage", "state-machine", "BoxTouchView", "GameMenuView", "TouchView", "UpgradView" ]);
