  ...
  BoxController: [ function(require, module, exports) {
    ...
    cc.Class({
      ...
      playHammers: function playHammers() {
        for (var i = 3; i < 7; i++) void 0 != this.hammers[i] && this.setSmPosition(i);
      },
      ...
      start: function start() {
        ...
        var _loop = function _loop(i) {
          ...
          nodehammer.setCompleteListener(function(trackEntry) {
            ...
            cc.log("HammerSpine [track %s][animation %s] end.", trackEntry.trackIndex, animationName);
            nodehammer.node.active = false;
            _this3.checkCanHammer(i) && _this3.smCallback(i);
          });
      ...
      resetBlockList: function resetBlockList() {
        if (this.blocks.length > 0) for (var i = 0; i < this.blocks.length; i++) for (var j = 0; j < this.blocks[i].length; j++) {
      ...
      cameraCentPosY: function cameraCentPosY() {
        ...
        if (oldY <= GameCenterY) return;
        var newY = oldY - this.blockWidth * move;
        var moveY = 0;
        moveY = newY >= GameCenterY ? newY : GameCenterY;
      ...
      setSmPosition: function setSmPosition(hammerpos) {
        ...
        for (;curline >= 0; curline--) {
          for (var i = 0; i < this.rowNum; i++) if (this.blocks[curline] && this.checkCanDestroy(curline, i)) {
            var _find = false;
            for (var j = 3; j <= 6; j++) if (this.HattingPos && this.HattingPos[j] && this.HattingPos[j].x == curline && i == this.HattingPos[j].y) {
              _find = true;
              break;
            }
      ...
      updateTouch: function updateTouch(point) {
        ...
        var gameY = this.gameNode.y;
        var realwidth = this.blockWidth + this.blockBlank;
        var line = Math.floor((point.y - (gameY - cameraY + this.BaseHeight - BaseGame)) / realwidth);
        var row = Math.floor((point.x - this.margins - this.blockBlank) / realwidth);
        var find = false;
        if (this.checkCanDestroy(line, row)) {
          for (var i = 3; i < 7; i++) if (null != this.HattingPos && null != this.HattingPos[i] && void 0 != this.HattingPos[i] && this.HattingPos[i].x == line && this.HattingPos[i].y == row) {
      ...
      changeHammerSpeed: function changeHammerSpeed() {
        var speedHammer = 0;
        var speedGold = 0;
        for (var i = 1; i <= 2; i++) if (Global["bar" + i] > 0) {
        ...
        for (var _i = 3; _i <= 6; _i++) if (void 0 != this.hammers[_i]) {
          var conf1 = AttributeConfig[Global.hammer[_i].attribute];
          this.hammers[_i].timeScale = conf1.att / parseFloat(conf1.time) * .17 * this.accelerHammer;
        }
      },
